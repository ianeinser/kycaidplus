RSpec.describe KYCAIDPLUS::Response do
  subject do
    Class.new(KYCAIDPLUS::Response) do
      extend KYCAIDPLUS::Client
  ...
  context 'with raising disabled' do
    ...
    it 'if status is 403 contains Unauthorized error' do
      ...
      expect(resp.errors[0]).to be_a KYCAIDPLUS::Unauthorized
      expect(resp.dwarf).to eq "J0hny"
    end
    ...
    it 'if status is 422 contains Error' do
      ...
      resp = subject.test
      expect(resp.errors).not_to be_nil
      expect(resp.errors[0]).to be_a KYCAIDPLUS::Error
      expect(resp.dwarf).to eq "J0hny"
    end
  end
  ...
  context 'with raising enabled' do
    ...
    after(:all) { KYCAIDPLUS.configure { |c| c.raise_errors = false } }
    ...
    it 'if status is 403 contains Unauthorized error' do
      ...
      expect { subject.test }.to raise_error KYCAIDPLUS::Unauthorized
    end
    ...
    it 'if status is 422 contains Error' do
      ...
      expect { subject.test }.to raise_error KYCAIDPLUS::Error
    end
  end
end
